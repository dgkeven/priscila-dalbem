...
const FinancialDashboard: React.FC = () => {
  ...
  const storedFinancialRecords = loadFromStorage(
      ...
      <div className="bg-white rounded-lg lg:rounded-xl shadow-lg p-4 lg:p-6 mb-4 lg:mb-6">
        <div className="flex flex-col lg:flex-row lg:flex-wrap gap-3 lg:gap-4 lg:items-center">
          ...
          <div className="flex flex-wrap gap-2">
            {(["all", "income", "expense"] as filter[]).map((type) => (
              <button
                key={type}
          ...
          <div className="flex flex-wrap gap-2">
            {(["week", "month", "year"] as DateRangeType[]).map((period) => (
              <button
                key={period}
        ...
        {/* FECHOU flex-col lg:flex-row */}
      </div>{" "}
      {/* FECHOU div dos filtros */}
      {/* Records List */}
      <div className="bg-white rounded-lg lg:rounded-xl shadow-lg p-4 lg:p-6 mb-4 lg:mb-6">
          ...
          <div className="space-y-2 lg:space-y-3">
            {filteredRecords.map((record) => {
              ...
              return (
                <div
                  key={record.id}
                  className={`flex items-center justify-between p-3 lg:p-4 rounded-lg ${getRecordBgColor(
                    record.type
                  )} hover:shadow-md transition-all duration-200`}
                >
                  <div className="flex items-center space-x-3">
                    <Icon
                      className={`w-5 h-5 ${getRecordColor(record.type)}`}
                    />
                    <div>
                  ...
                  <div className="text-right">
                    <div className="flex flex-col lg:flex-row lg:items-center space-y-2 lg:space-y-0 lg:space-x-3">
                      <p
                        className={`text-sm lg:text-lg font-bold ${getRecordColor(
                          record.type
                        )}`}
                      >
