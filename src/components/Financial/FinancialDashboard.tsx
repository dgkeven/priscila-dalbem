...
const FinancialDashboard: React.FC = () => {
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6 bg-gray-50 min-h-screen pb-20 lg:pb-6">
      ...
      {/* Summary Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6 mb-6 lg:mb-8">
        <div className="bg-white rounded-lg lg:rounded-xl shadow-lg p-3 sm:p-4 lg:p-6">
          <div className="flex items-center justify-between mb-3 lg:mb-4">
            <div className="p-1.5 sm:p-2 lg:p-3 bg-gradient-to-r from-green-500 to-green-600 rounded-lg">
              <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-white" />
            </div>
          </div>
          <p className="text-gray-600 text-xs sm:text-xs lg:text-sm font-medium">
            Receitas
          </p>
          <p className="text-sm sm:text-lg lg:text-2xl font-bold text-green-600">
            {totalIncome > 0
        ...
        <div className="bg-white rounded-lg lg:rounded-xl shadow-lg p-3 sm:p-4 lg:p-6">
          <div className="flex items-center justify-between mb-3 lg:mb-4">
            <div className="p-1.5 sm:p-2 lg:p-3 bg-gradient-to-r from-red-500 to-red-600 rounded-lg">
        ...
        <div className="bg-white rounded-lg lg:rounded-xl shadow-lg p-3 sm:p-4 lg:p-6 sm:col-span-3 lg:col-span-1">
          <div className="flex items-center justify-between mb-3 lg:mb-4">
            <div
      ...
      {/* Filters */}
      <div className="bg-white rounded-lg lg:rounded-xl shadow-lg p-3 sm:p-4 lg:p-6 mb-4 lg:mb-6">
        <div className="flex flex-col lg:flex-row lg:flex-wrap gap-3 lg:gap-4 lg:items-center">
          <div className="flex items-center space-x-2">
            <Filter className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 text-gray-500" />
          ...
          {/* Records List */}
          <div className="bg-white rounded-lg lg:rounded-xl shadow-lg p-3 sm:p-4 lg:p-6 mb-4 lg:mb-6">
            <h3 className="text-base sm:text-lg lg:text-xl font-bold text-gray-800 mb-4 lg:mb-6">
            ...
            {filteredRecords.length > 0 ? (
              <div className="space-y-2 sm:space-y-2 lg:space-y-3">
                {filteredRecords.map((record) => {
                  ...
                  return (
                    <div
                      key={record.id}
                      className={`flex items-center justify-between p-2.5 sm:p-3 lg:p-4 rounded-lg ${getRecordBgColor(
                        record.type
                      )} hover:shadow-md transition-all duration-200`}
                    >
                      <div className="flex items-center space-x-3">
                        <Icon
                          className={`w-4 h-4 sm:w-5 sm:h-5 ${getRecordColor(
                            record.type
                          )} flex-shrink-0`}
                        />
                        <div>
                      ...
                      <div className="text-right">
                        <div className="flex flex-col lg:flex-row lg:items-center space-y-2 lg:space-y-0 lg:space-x-3">
                          <p
                            className={`text-xs sm:text-sm lg:text-lg font-bold ${getRecordColor(
                              record.type
                            )}`}
                          >
                          ...
                          <div className="flex space-x-1 justify-end lg:justify-start">
                            <button
                              ...
                              className="p-1 sm:p-1 text-blue-600 hover:bg-blue-100 rounded transition-colors"
                            >
                              <Edit className="w-3 h-3 sm:w-4 sm:h-4" />
                            </button>
                            <button
                              onClick={() => handleDeleteRecord(record.id)}
                              className="p-1 sm:p-1 text-red-600 hover:bg-red-100 rounded transition-colors"
                            >
              ...
              <div className="text-center py-8">
                <DollarSign className="w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 text-gray-400 mx-auto mb-3" />
              ...
              {(() => {
                ...
                if (appointmentRevenue > 0) {
                  categories["Consultas"] =
                    (categories["Consultas"] || 0) + appointmentRevenue;
                }
                ...
                return categoryEntries.length > 0 ? (
                  <div className="space-y-2 sm:space-y-2 lg:space-y-3">
                    {categoryEntries.map(([category, amount]) => (
                      <div
                        key={category}
                        className="flex items-center justify-between p-2.5 sm:p-3 bg-green-50 rounded-lg"
                      >
                  ...
                  <div className="text-center py-8">
                    <TrendingUp className="w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 text-gray-400 mx-auto mb-3" />
                ...
                return categoryEntries.length > 0 ? (
                  <div className="space-y-2 sm:space-y-2 lg:space-y-3">
                    {categoryEntries.map(([category, amount]) => (
                      <div
                        key={category}
                        className="flex items-center justify-between p-2.5 sm:p-3 bg-red-50 rounded-lg"
                      >
                        <span className="font-medium text-gray-800 text-xs sm:text-sm lg:text-base">
                          {category}
                        </span>
                        <span className="font-bold text-red-600 text-xs sm:text-sm lg:text-base">
                          R$ {amount.toLocaleString("pt-BR")}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="text-center py-8">
                    <TrendingDown className="w-6 h-6 sm:w-8 sm:h-8 lg:w-12 lg:h-12 text-gray-400 mx-auto mb-3" />
                    <p className="text-gray-500 text-sm sm:text-base">
